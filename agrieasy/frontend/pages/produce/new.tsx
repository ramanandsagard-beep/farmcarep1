...
export default function NewProduce() {
  ...
  const uploadImage = async (): Promise<string | null> => {
    ...
    try {
      const res = await api('/produce/upload-image', { method: 'POST', body: formData }, token);
      return res.imageUrl;
    } catch (e) {
  ...
  const create = async () => {
    ...
    try {
      await api('/produce', { method: 'POST', body: JSON.stringify({ name, price, stock, imageUrl }) }, token);
  ...
  return (
    <div className="max-w-md mx-auto bg-white p-6 rounded shadow">
      <h1 className="text-xl font-semibold mb-4">List Produce</h1>
